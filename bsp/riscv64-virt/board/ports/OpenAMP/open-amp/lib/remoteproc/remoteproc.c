...
//OPENAMP_HACK_RSC: hack resource table
#ifdef OPENAMP_HACK_RSC
...
struct shared_resource_table rsc;

static int remoteproc_virtio_notify(void *priv, uint32_t id)
{
	printf("TODO: notify: %d: id\n", id);
}
#endif /* #ifdef OPENAMP_HACK_RSC */
...
#define VRING_RX_ADDRESS     0x10040000     /* allocated by Master processor */
#define VRING_TX_ADDRESS     0x10040400     /* allocated by Master processor */
...
#define VRING_NUM_BUFFS      16             /* number of rpmsg buffer */
	...
	rsc.vring0.da = VRING_TX_ADDRESS;
	rsc.vring0.align = VRING_ALIGNMENT;
	rsc.vring0.num = VRING_NUM_BUFFS;
	rsc.vring0.notifyid = VRING0_ID;

	rsc.vring1.da = VRING_RX_ADDRESS;
	rsc.vring1.align = VRING_ALIGNMENT;
	rsc.vring1.num = VRING_NUM_BUFFS;
	rsc.vring1.notifyid = VRING1_ID;

	rsc.vdev.type = RSC_VDEV;
	rsc.vdev.id = VIRTIO_ID_RPMSG_;
	rsc.vdev.num_of_vrings=VRING_COUNT;
