    ...
    uintptr_t (*prepare_page_for_va)(uintptr_t va, uintptr_t pgdir))
{
    ...
    while (ph_entry_count--) {
        ...
        Elf64_Addr tmp,vaddr = phdr->p_vaddr;
        Elf64_Off offset = phdr->p_offset;
        Elf64_Xword file_size = phdr->p_filesz;
        Elf64_Xword mem_size = phdr->p_memsz;
        int loop_cnt = mem_size / NORMAL_PAGE_SIZE + (mem_size % NORMAL_PAGE_SIZE > 0);
        if (phdr->p_type == PT_LOAD) {
            /* TODO: */
            // load it into memory
            for (int i = 0; i < loop_cnt; ++i) {
                if (i == 0) {
                    tmp = (* prepare_page_for_va)(vaddr, pgdir);
                    
                } else {
                    tmp = (* prepare_page_for_va)(vaddr + i * NORMAL_PAGE_SIZE, pgdir);
                }
                if (file_size > PAGE_SIZE) {
                        memcpy((void *)tmp, elf_binary + offset, PAGE_SIZE);
                        offset += PAGE_SIZE;
                        file_size -= PAGE_SIZE;
                    } else
                    {
                       memcpy((void *)tmp, elf_binary + offset, file_size);
                    }
            }

            //memset(tmp + file_size, 0, mem_size - file_size);
        }
